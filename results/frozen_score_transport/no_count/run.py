"""Ablate observed-source count from the frozen m=1 shared score."""
...
def feature_matrix(probabilities: np.ndarray, observed_count: np.ndarray) -> np.ndarray:
    ...
    return np.column_stack((probabilities, entropy, ordered[:, 0], ordered[:, 0] - ordered[:, 1]))
...
def fit_frozen_models(
    ...
    for outer in range(5):
        ...
        x_train = np.asarray(
            [controls.posterior_features(row, observed[str(row["record_id"])])[:-1] for row in train]
        )
        y_train = np.asarray(
        ...
        summaries.append(
            {
                ...
                "score_variant": "posterior_features_without_observed_source_count",
...
def main() -> None:
    ...
    for multiplicity in MULTIPLICITIES:
        ...
        if multiplicity == 1:
            ...
            if not np.isfinite(pact_anchor) or not np.isfinite(nested_anchor):
                raise AssertionError("non-finite no-count score anchor")
            m1_ncs = {"pact_registered": pact_anchor, "nested_unaware": nested_anchor}
    ...
    gate = {
        ...
        "score_variant": "posterior features without observed-source count",
        ...
        "verifier": "disabled",
        "m1_pact_shared_score_ncsaurc": m1_ncs["pact_registered"],
        "m1_nested_shared_score_ncsaurc": m1_ncs["nested_unaware"],
